				...
				<div class="fallback-toast">
					<div class="fallback-toast-left">
						...
						<p class="fallback-toast-msg">
							<strong>AI scoring temporarily unavailable</strong> &mdash; the AI service is
							currently busy. Your scores below are still accurate (rule-based analysis), but the AI
							suggestions will be less specific. Please try again later.
				...
				<div class="suggestions-cards">
					{#each allSuggestions as suggestion, i}
						...
						{@const impactColor = structured
							? (impactColorMap[suggestion.impact] ?? '#8a6500')
...
<style>
	...
	.mode-badge {
		...
		border: 1px solid rgba(116, 206, 0, 0.2);
		border-radius: 999px;
		font-size: 0.75rem;
		font-weight: 600;
	...
	.quick-wins-band {
		...
		background:
			linear-gradient(135deg, rgba(116, 206, 0, 0.04), rgba(46, 111, 64, 0.03)), var(--glass-bg);
		border: 1px solid rgba(116, 206, 0, 0.18);
	...
	.quick-wins-rank {
		...
		font-weight: 700;
		color: var(--accent-cyan);
		background: rgba(116, 206, 0, 0.1);
		border: 1px solid rgba(116, 206, 0, 0.25);
		border-radius: 50%;
	}
	...
	.suggestion-card:hover {
		...
		background: rgba(116, 206, 0, 0.03);
	}
	...
	.copy-btn:hover {
		background: rgba(16, 17, 18, 0.08);
		border-color: rgba(16, 17, 18, 0.16);
		color: var(--text-secondary);
	}
