<script lang="ts">
	...
	const shareText = $derived(
		`Just scored ${avgScore}/100 on Zylto, a free resume tool that simulates how real ATS platforms (Workday, Taleo, iCIMS, Greenhouse, Lever, and SuccessFactors) parse your resume.\n\n${passCount}/${totalCount} systems passed. Try it free at zylto-ats-resume-checker.vercel.app\n\n#Zylto #Resume #JobSearch #CareerTips`
	);
		...
		img.onload = () => {
			...
			canvas.toBlob((blob) => {
				...
				const dateStr = d
					...
					.replace(',', '');
				a.download = `${displayName} - Zylto Badge - ${dateStr}.png`;
				a.click();
	...
	function shareToTwitter() {
		...
		const tight = `Just scored ${avgScore}/100 on Zylto (free, simulates how Workday, Lever, iCIMS and others actually parse resumes). ${passCount}/${totalCount} systems passed.`;
	...
	function addToLinkedInProfile() {
		...
		const url = `https://www.linkedin.com/profile/add?startTask=CERTIFICATION_NAME&name=${encodeURIComponent('ATS Screening Report')}&organizationName=${encodeURIComponent('Zylto')}&issueYear=${year}&issueMonth=${month}&certUrl=${encodeURIComponent('https://zylto-ats-resume-checker.vercel.app')}`;
		window.open(url, '_blank', 'noopener,noreferrer');
	}
</script>
					...
					>
						<defs>
							<linearGradient id="badge-bgGrad" x1="0" y1="0" x2="0.5" y2="1">
								...
								<stop offset="100%" stop-color="#eef1f3" />
							</linearGradient>
							<linearGradient id="badge-accentGrad" x1="0" y1="0" x2="1" y2="1">
								<stop offset="0%" stop-color="#467b00" />
								<stop offset="50%" stop-color="#a8ff24" />
								<stop offset="100%" stop-color="#2e6f40" />
							</linearGradient>
							<linearGradient id="badge-borderGrad" x1="0" y1="0" x2="1" y2="1">
								<stop offset="0%" stop-color="#467b00" stop-opacity="0.6" />
								<stop offset="25%" stop-color="#a8ff24" stop-opacity="0.2" />
								<stop offset="50%" stop-color="#2e6f40" stop-opacity="0.6" />
								<stop offset="75%" stop-color="#a8ff24" stop-opacity="0.2" />
								<stop offset="100%" stop-color="#467b00" stop-opacity="0.6" />
							</linearGradient>
						...
						<g opacity="0.025" clip-path="url(#badge-cardClip)">
							{#each Array(18) as _, i}
								<line x1="4" y1={i * 42} x2="516" y2={i * 42} stroke="#101112" stroke-width="0.5" />
								<line
									x1={i * 30 + 4}
									y1="4"
									x2={i * 30 + 4}
						...
						<circle cx="480" cy="50" r="100" fill="#467b00" opacity="0.025" />
						<circle cx="40" cy="670" r="80" fill="#2e6f40" opacity="0.025" />
						...
						<g transform="translate(260, 52)">
							...
							<text
								...
								text-anchor="start"
								letter-spacing="0.1em">ZYLTO</text
							>
						</g>

						<!-- thin divider -->
						<line x1="80" y1="84" x2="440" y2="84" stroke="rgba(16,17,18,0.06)" stroke-width="1" />

						<!-- verification seal watermark (top right) -->
						<g transform="translate(430, 130)" opacity="0.06">
							<circle cx="0" cy="0" r="36" fill="none" stroke="#101112" stroke-width="1" />
							<circle cx="0" cy="0" r="28" fill="none" stroke="#101112" stroke-width="0.5" />
						...
						<g transform="translate(260, 365)">
							<rect
								...
								height="28"
								rx="14"
								fill="rgba(16,17,18,0.04)"
								stroke="rgba(16,17,18,0.08)"
								stroke-width="1"
							/>
							<text
								x="0"
						...
						<g transform="translate(260, 510)">
							...
							<line x1="0" y1="-8" x2="0" y2="24" stroke="rgba(16,17,18,0.06)" stroke-width="1" />
							<g transform="translate(90, 0)">
								<text
									x="0"
									y="0"
							...
							{#each scoresStore.results as result, i}
								...
								<g transform="translate({x}, {y})">
									<rect
										...
										height="22"
										rx="6"
										fill="rgba(16,17,18,0.02)"
										stroke="rgba(16,17,18,0.03)"
										stroke-width="0.5"
									/>
									<text
										x="-54"
						...
						<text
							...
							fill="#467b00"
							text-anchor="middle"
							opacity="0.6">zylto-ats-resume-checker.vercel.app</text
						>
...
<style>
	...
	.action-btn.primary {
		background: rgba(116, 206, 0, 0.12);
		border-color: rgba(116, 206, 0, 0.25);
		color: #467b00;
	}

	.action-btn.primary:hover {
		background: rgba(116, 206, 0, 0.2);
		border-color: rgba(116, 206, 0, 0.4);
		box-shadow: 0 0 20px rgba(116, 206, 0, 0.1);
	}
	...
	.action-btn.twitter:hover {
		background: rgba(16, 17, 18, 0.1);
		border-color: rgba(16, 17, 18, 0.22);
		box-shadow: 0 0 18px rgba(16, 17, 18, 0.05);
	}
