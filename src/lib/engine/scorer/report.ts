...
export async function generatePDF() {
	...
	tc(P.light);
	wrappedText(
		'This report is provided for informational purposes only. Zylto is an independent open-source tool and is not affiliated with any ATS vendor.',
		ML,
		footerY,
		68,
		2.8
	...
	doc.setFontSize(6.5);
	tc(P.dark);
	doc.text('zylto-ats-resume-checker.vercel.app', W - MR, footerY, { align: 'right' });
	doc.setFont('helvetica', 'normal');
	doc.setFontSize(5);
	tc(P.light);
